...
def preemp(input, p=0.97):
    ...
    return np.array([lfilter(b, a, frame) for frame in input])
...
def windowing(input):
    ...
    """
    num_samples = input.shape[1]  # Number of samples in each frame
    hamming_window = hamming(num_samples, sym=False)
...
def logMelSpectrum(input, sampling_rate,nfft):
    """
    ...
    fbank = trfbank(sampling_rate,nfft)
    
    # Apply the filters to the power spectrum
    mspec = np.log(np.dot(input, fbank.T))
