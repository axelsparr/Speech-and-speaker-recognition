...
def wer(reference, hypothesis, ignore_case=False, delimiter=' '):
	...
	if isinstance(reference,list):
		reference = delimiter.join(reference)
	if isinstance(hypothesis, list):
		hypothesis = delimiter.join(hypothesis)
...
def cer(reference, hypothesis, ignore_case=False, remove_space=False):
	...
	if isinstance(reference, list):
		reference = join_char.join(reference)
	if isinstance(hypothesis, list):
		hypothesis = join_char.join(hypothesis)
...
def test(model, device, test_loader, criterion, epoch):
	...
	with torch.no_grad():
		for _data in tqdm(test_loader):
			...
			# get predicted text
			decoded_preds = []
			if args.decoder == "wiki":
				for k in range(len(labels)):
					batch_decoded_preds = decoder.decode(output[k].cpu().detach().numpy())
					#convert it to our indexing
					decoded_preds.append(intToStr(strToInt(batch_decoded_preds)))
			else:
...
def validate(model, device, val_loader, criterion, alpha,beta):
	print('\nevaluating…')
	decoder = build_ctcdecoder(
		[x for x in "' abcdefghijklmnopqrstuvwxyz"],
		kenlm_model_path="wiki-interpolate.3gram.arpa",
...
if __name__ == '__main__':
	...
	argparser.add_argument('--decoder', type=str,help='what decoder to use, greedy or language model (wiki)')
	...
	if args.mode == 'train':
		...
		for wavfile in args.wavfiles:
			...
			if args.decoder == "greedy":
				...
				text = decoder.decode(output.squeeze(0).cpu().detach().numpy())
				#convert it to our indexing
				text = intToStr(strToInt(text))
			else:
				print("Invalid decode argument valid ones are: greedy or wiki")
		...
		if args.decoder == "greedy":
			...
			with torch.no_grad():
				for I, _data in enumerate(test_loader):
					...
					decoded_preds = decoder.decode(output.squeeze(0).cpu().detach().numpy())
					#convert it to our indexing
					decoded_preds = intToStr(strToInt(decoded_preds))
					# get predicted text
					#decoded_preds = greedyDecoder(output)
...
#best alpha,beta is: 1,1 with min wer: 2.593229657003127
	...
	#Best result after 10 epoch: evaluating… Test set: Average loss: 0.6026, Average CER: 0.139811 Average WER: 0.1699
